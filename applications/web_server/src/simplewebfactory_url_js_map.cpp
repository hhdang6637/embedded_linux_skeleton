...
void simpleWebFactory::init_url_js_map()
{
    ...
    this->url_js_map.insert(std::pair<std::string, jsCallback>("/json/openvpn_cfg_server", json_handle_openvpn_cfg_server));
    this->url_js_map.insert(std::pair<std::string, jsCallback>("/json/openvpn_cfg_client", json_handle_openvpn_cfg_client));
#if defined (pi_3_b) || defined (orange_pi_zero)
