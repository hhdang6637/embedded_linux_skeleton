...
#define OPENVPN_CONF "/tmp/confg/"  // this will be generated by this module
...
static bool openVpnManager_generate_openvpncfg(void) {
    ...
    std::ofstream openvpn_conf_file(OPENVPN_CONF"openvpn.conf");
...
static bool openVpnManager_start_openvpn_service(void) {
    ...
    if (access(OPENVPN_CONF"openvpn.conf", F_OK) == -1) {
        syslog(LOG_ERR, OPENVPN_CONF"openvpn.conf not found");
        return false;
    }
    ...
    snprintf(openvpn_cmd, sizeof(openvpn_cmd),
        ...
        "--daemon --writepid /var/run/openvpn.pid", OPENVPN_CONF"openvpn.conf");
